...
@section('title')
    Outreach Link Agency | Explore Site Order View
@endsection
                            ...
                            @if ($exploreserviceorder->orderDetails->service->name != 'Link Insertion')
                                <div class="col-12">
                                    <div class="page-title-box d-flex align-items-center justify-content-between">
                                        <span>Service buy content:
                                            @foreach ($exploreserviceorder->orderDetails->service->serviceContentType as $item)
                                                @if (!is_null($exploreserviceorder->wordLength))
                                                    @if ($item->name === 'Content Writing & Publication')
                                                        <b>{{ $item->name }}</b>
                                                    @endif
                                                @else
                                ...
                                @if (!is_null($exploreserviceorder->wordLength))
                                    <div class="col-12">
                                        <div class="page-title-box d-flex align-items-center justify-content-between">
                                            <span>Word Length:
                                                <b>{{ $exploreserviceorder->wordLength->length }}</b>
                                            </span>
                                        </div>
                                    </div>
                                @endif
